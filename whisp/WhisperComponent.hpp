...
//
#ifndef WhisperComponent_hpp
#define WhisperComponent_hpp
...
//  500 -> 00:05.000
// 6000 -> 01:00.000
inline std::string to_timestamp(int64_t t, bool comma = false) {
    int64_t msec = t * 10;
...
inline int timestamp_to_sample(int64_t t, int n_samples) {
    return std::max(0, std::min((int) n_samples - 1, (int) ((t*WHISPER_SAMPLE_RATE)/100)));
}

// helper function to replace substrings
inline void replace_all(std::string & s, const std::string & search, const std::string & replace) {
...
inline void whisper_print_segment_callback(struct whisper_context * ctx, int n_new, void * user_data) {
...
inline bool output_txt(struct whisper_context * ctx, const char * fname) {
...
inline bool output_vtt(struct whisper_context * ctx, const char * fname) {
    std::ofstream fout(fname);
    if (!fout.is_open()) {
        fprintf(stderr, "%s: failed to open '%s' for writing\n", __func__, fname);
        return false;
...
inline bool output_srt(struct whisper_context * ctx, const char * fname, const whisper_params & params) {
    std::ofstream fout(fname);
    if (!fout.is_open()) {
        fprintf(stderr, "%s: failed to open '%s' for writing\n", __func__, fname);
        return false;
...
inline bool output_csv(struct whisper_context * ctx, const char * fname) {
    std::ofstream fout(fname);
    if (!fout.is_open()) {
        fprintf(stderr, "%s: failed to open '%s' for writing\n", __func__, fname);
        return false;
...
inline bool output_wts(struct whisper_context * ctx, const char * fname, const char * fname_inp, const whisper_params & /*params*/, float t_sec) {
    std::ofstream fout(fname);
